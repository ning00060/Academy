    ...
    <div class="homeMainDiv">
        ...
        <table>
            <c:forEach var="notice" items="${noticeList}">
                <tr>
                    <td><a href="${pageContext.request.contextPath}/user/notice?id=${notice.id}">${notice.category} ${notice.title}</a></td>
                    <td><fmt:formatDate value="${notice.createdTime}" type="date" /></td>
                    <c:if test="${verifiedUser.permissionLevel == 3}">
                        <td><a class="btn-edit" href="${pageContext.request.contextPath}/board/edit?id=${notice.id}">수정</a></td>
                        <td><a class="btn-delete" href="${pageContext.request.contextPath}/board/delete?id=${notice.id}">삭제</a></td>
                    </c:if>
                </tr>
            </c:forEach>
        </table>
