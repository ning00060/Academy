...
public class ProfessorController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 강의목록 조회하기 폼에서 개설년도, 개설학기 정보를 받아 내 강의 목록 조회 후 내 강의 목록 폼으로 이동
		case "/mysubject":
		...
		// 내 강의 목록 조회 폼에서 개설년도 개설학기 정보를 얻은 후 해당 정보로 내 강의 리스트 폼으로 이동
...
>>>>>>> dev2
		...
		case "/gohome":
			response.sendRedirect("/Academy/user/home");
			// request.getRequestDispatcher("/Academy/user/home").forward(request,
			// response);
			break;
	...
	private void deleteHopeClass(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
	...
	private void restclassmanagement(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
	...
	private void clickERMenu(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			HttpSession session = request.getSession();
			ProfessorDTO professor = (ProfessorDTO) session.getAttribute("professorDTO");
	...
	private void goinputpage(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			HttpSession session = request.getSession();
			ProfessorDTO professor = (ProfessorDTO) session.getAttribute("professorDTO");
	...
	private void addHopeClass(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
	...
	private void readHopeClassList(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			HttpSession session = request.getSession();
			ProfessorDTO professor = (ProfessorDTO) session.getAttribute("professorDTO");
	...
	private void inputHopeClass(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			int professorId = Integer.parseInt(request.getParameter("professorId"));
			...
			PrintWriter pw = response.getWriter();
			pw.println("<script>");
			pw.println("alert('잘못된 입력입니다. 다시 입력해 주세요.');");
			pw.println("history.back();");
			pw.println("</script>");
	...
	private void inputUpdateRC(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			int id = Integer.parseInt(request.getParameter("id"));
	...
	private void deleteRC(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			int id = Integer.parseInt(request.getParameter("id"));
	...
	private void updateRC(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			int id = Integer.parseInt(request.getParameter("id"));
	...
	private void movemodifygrade(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
	...
	private void resultCheck(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			String subjectId = request.getParameter("subjectId");
			String subjectName = request.getParameter("subjectName");
			List<EvaluationResultDTO> erList = professorRepository
					.selectEvaluationResultBySubjectId(Integer.parseInt(subjectId));
	...
	private void readMyEvaluationResult(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			String year = request.getParameter("year");
			String semester = request.getParameter("semester");
			int professorId = Integer.parseInt(request.getParameter("professorId"));
			List<SubjectDTO> subjectList = professorRepository.selectAllSubjectByProfessorIdYearSemester(professorId,
					Integer.parseInt(year), Integer.parseInt(semester));
	...
	private void inputRestClass(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			...
			int year = Integer.parseInt(request.getParameter("year"));
			int semester = Integer.parseInt(request.getParameter("semester"));
			List<SubjectDTO> subjectList = professorRepository.selectAllSubjectByProfessorIdYearSemester(professorId, year,
					semester);
			...
			String restDay = request.getParameter("restDay");
			String supplement = request.getParameter("supplement");
			professorRepository.insertRestClass(subjectId, subjectName, professorId, restDay, roomId, year, semester,
					supplement);
			request.getRequestDispatcher("/WEB-INF/views/professor/selectRC.jsp").forward(request, response);
		} catch (Exception e) {
			response.setContentType("text/html; charset=UTF-8");
	...
	private void moveInputForm(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			String year = request.getParameter("year");
			String semester = request.getParameter("semester");
			...
			List<SubjectDTO> subjectList = professorRepository.selectAllSubjectByProfessorIdYearSemester(
					Integer.parseInt(professorId), Integer.parseInt(year), Integer.parseInt(semester));
	...
	private void readMySubjectRC(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
	...
	private void enterStudentGrade(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int i = 1; i <= studentSize; i++) {
				...
				finalExam = Integer.parseInt(request.getParameter("finalExamScore" + i));
				convertedMark = Float.parseFloat(request.getParameter("grade" + i));
				professorRepository.insertStudentsGradesByStudentId(studentId, subjectId, midExam, finalExam,
						convertedMark);
				}
	...
	private void readMySubject(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		try {
			String year = request.getParameter("year");
			String semester = request.getParameter("semester");
